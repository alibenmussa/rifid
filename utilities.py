...
def protected_media(request, path: str, document_root=settings.MEDIA_ROOT, show_indexes=False, is_api=False):
...
def upload_to_directory(route, filename):
    return '{0}/{1}.{2}'.format(route, uuid.uuid4().hex[:16], filename.split('.')[-1])
...
class Button:
    def __init__(self, url=None, params=None, label=None, icon=None, style="", attrs=""):
        ...
        self.label = label or ""
        self.icon = icon or "fa fa-edit"
        self.style = style
        self.attrs = attrs

    def render(self):
        html = ""
        icon = f'<i class="fa align-middle {self.icon}"></i>'
        if "btn-" not in self.style:
            self.style = "btn-primary " + self.style
        if self.url:
            url = '{% url "' + self.url + '" ' + self.params + ' %}'
            html += f'<a href="{url}" class="btn btn-sm {self.style}" {self.attrs}>'
            html += icon
